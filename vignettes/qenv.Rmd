...
Consider a case when a base::subset of `iris` is defined by an input value.
```{r}
q <- qenv()
q <- eval_code(q, quote(i <- base::subset(iris, Species == "setosa")))
q <- eval_code(q, substitute(
  ii <- base::subset(iris, Species == species),
  env = list(species = "versicolor")
))
input_value <- "virginica"
q <- eval_code(q, substitute(
  iii <- base::subset(iris, Species == species),
...
```{r}
qq <- qenv()
qq <- within(qq, i <- base::subset(iris, Species == "setosa"))
qq <- within(qq, ii <- base::subset(iris, Species == species), species = "versicolor")
input_value <- "virginica"
qq <- within(qq, iii <- base::subset(iris, Species == species), species = input_value)
