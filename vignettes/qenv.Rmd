...
The code tracked by `qenv` may contain sensitive information that you may want to mask or replace with safe information before publishing. This can be achieved with the `replace_code` function that replaces the most recent code block stored in a `qenv` with supplied code.

```{r}
# simulate a data base connection
q_connector <- eval_code(new_qenv(), quote(
...
q_sensitive <- eval_code(q_connector, quote(connect("user", "pass")))
get_code(q_sensitive, deparse = TRUE) # credentials exposed
# replace connection code
q_masked <- replace_code(q_sensitive, quote(connect(readline("enter user name: "), readline("enter password: "))))
get_code(q_masked)
```
