...
Note that there is a public [shinymeta](https://github.com/rstudio/shinymeta) R package from RStudio that offers similar functionality. However, `shinymeta` is currently not easily integratable into `teal` modules and hence we recommend using `chunks` for `teal` based apps
...
2. Pre-processing Code: refers to the code which precedes the teal app initialization, this includes:
  - Data imports
  - Data transformation
...
The header is created by `teal`, the pre-processing code must be supplied by the app developer and the analysis code is provided by the teal modules.
...
3. The code used for merging multiple data sets - by `teal.transform::data_extract_srv`
